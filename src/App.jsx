...
export default function App() {
  ...
  const loadMovieSection = useCallback(async () => {
    ...
    } catch (e) { console.error("Movie section error:", e); setMovieData(d => ({ ...d, loading:false })); }
  }, [movieData.loaded, movieData.loading]);

  const loadMangaSection = useCallback(async () => {
    if (mangaData.loaded || mangaData.loading) return;
    ...
    } catch (e) { console.error("Manga section error:", e); setMangaData(d => ({ ...d, loading:false })); }
  }, [mangaData.loaded, mangaData.loading]);
    ...
    <div style={{ minHeight:"100vh",background:"#000",color:"#e2e8f0",overflowX:"hidden" }}>
      ...
      <footer style={{ borderTop:"1px solid #140d28",padding:"26px 20px",textAlign:"center" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:6 }}>
          <Logo size={20}/>
          <span style={{ fontSize:14,fontWeight:900,letterSpacing:2,textTransform:"uppercase",color:"#fff" }}>SHIN<span style={{ background:"linear-gradient(135deg,#7c3aed,#c084fc)",padding:"0 2px",borderRadius:2 }}>O</span>RA</span>
        </div>
