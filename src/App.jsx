...
══════════════════════════════════════════ */
/* ═══════════════════════════════════════════════════════
   MOVIE API  (TMDB — themoviedb.org)
   111movies uses TMDB IDs directly for embeds.
   Add your free TMDB key in Vercel:
     Settings → Environment Variables → VITE_TMDB_KEY
...
// Normalize a TMDB movie into our app's expected shape
const normMovie = (m) => ({
  ...m,
  large_cover_image : m.poster_path   ? `${TMDB_IMG}${m.poster_path}`   : null,
  background_image  : m.backdrop_path ? `${TMDB_BACK}${m.backdrop_path}` : null,
  rating            : m.vote_average  ? m.vote_average.toFixed(1) : "N/A",
  year              : m.release_date  ? m.release_date.split("-")[0] : "",
  description_full  : m.overview || "",
  genres            : [],  // genre_ids resolved server-side; display is handled separately
  // id is already the TMDB id — used directly by 111movies & vidsrc
});

const fetchMovies = async ({ page=1, search="", genre="All" }={}) => {
  if (!TMDB_KEY) return { list:[], hasMore:false, page };
  try {
    let url;
    if (search.trim()) {
      url = `${TMDB_BASE}/search/movie?api_key=${TMDB_KEY}&query=${encodeURIComponent(search)}&page=${page}&include_adult=false`;
    } else if (genre !== "All" && TMDB_GENRE_IDS[genre]) {
      url = `${TMDB_BASE}/discover/movie?api_key=${TMDB_KEY}&with_genres=${TMDB_GENRE_IDS[genre]}&sort_by=popularity.desc&page=${page}`;
    } else {
      url = `${TMDB_BASE}/movie/popular?api_key=${TMDB_KEY}&page=${page}`;
    }
    const r = await timedFetch(url);
    const d = await r.json();
    if (d.status_code === 7) return { list:[], hasMore:false, page }; // invalid key
    return { list:(d.results||[]).map(normMovie), hasMore: page < (d.total_pages||1), page };
  } catch(e) { console.error("Movies error:", e); return { list:[], hasMore:false, page }; }
};

const fetchMovieRow = async (endpoint) => {
  if (!TMDB_KEY) return [];
  try {
    const r = await timedFetch(`${TMDB_BASE}${endpoint}&api_key=${TMDB_KEY}`);
    const d = await r.json();
    return (d.results||[]).map(normMovie);
  } catch(e) { console.error("Movie row error:", e); return []; }
};
...
const MOVIE_SERVERS = [
  { id:"m1", label:"Server 1", tag:"HD",  url:(m)=>`https://111movies.net/movie/${m.id}` },
  { id:"m2", label:"Server 2", tag:"HD",  url:(m)=>`https://vidsrc.to/embed/movie/${m.id}` },
  { id:"m3", label:"Server 3", tag:"HD",  url:(m)=>`https://vidsrc.xyz/embed/movie?tmdb=${m.id}` },
  { id:"m4", label:"Server 4", tag:"ALT", url:(m)=>`https://vidsrc.me/embed/movie?tmdb=${m.id}` },
];
...
function TmdbBanner() {
  ...
  return (
    <div style={{background:"linear-gradient(135deg,rgba(124,58,237,.15),rgba(168,85,247,.08))",border:"1px solid rgba(168,85,247,.3)",borderRadius:12,padding:"18px 22px",marginBottom:28,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
      <div>
        <div style={{color:"#c084fc",fontWeight:800,fontSize:14,marginBottom:4}}>⚙️ TMDB API Key Required for Movies</div>
        <div style={{color:"#6b7280",fontSize:13}}>Get your free key at <strong style={{color:"#a855f7"}}>themoviedb.org/settings/api</strong>, then add it in Vercel → Settings → Environment Variables as <strong style={{color:"#a855f7"}}>VITE_TMDB_KEY</strong></div>
      </div>
    </div>
  );
}
      ...
      <div style={{ display:"flex",alignItems:"center",gap:9,flexShrink:0,cursor:"pointer" }} onClick={() => setSection("anime")}>
        ...
        <div>
          <div style={{ fontSize:15,fontWeight:900,letterSpacing:2,textTransform:"uppercase",color:"#fff",lineHeight:1 }}>
            SHIN<span style={{ display:"inline-block",color:"#c084fc",border:"2px solid #a855f7",borderRadius:0,padding:"0 3px",position:"relative",top:"-1px",letterSpacing:0,fontWeight:900 }}>O</span>RA
          </div>
  ...
  const loadMovieSection = useCallback(async () => {
    ...
    try {
      const [pop, top, act, anim] = await Promise.all([
        fetchMovieRow("/movie/popular?page=1"),
        fetchMovieRow("/movie/top_rated?page=1"),
        fetchMovieRow("/discover/movie?with_genres=28&sort_by=popularity.desc&page=1"),
        fetchMovieRow("/discover/movie?with_genres=16&sort_by=popularity.desc&page=1"),
      ]);
    ...
    <div style={{ minHeight:"100vh",background:"#000",color:"#e2e8f0",overflowX:"hidden" }}>
      ...
      <footer style={{ borderTop:"1px solid #140d28",padding:"26px 20px",textAlign:"center" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:6 }}>
          <Logo size={20}/>
          <span style={{ fontSize:14,fontWeight:900,letterSpacing:2,textTransform:"uppercase",color:"#fff" }}>SHIN<span style={{ display:"inline-block",color:"#c084fc",border:"2px solid #a855f7",borderRadius:0,padding:"0 2px",position:"relative",top:"-1px",letterSpacing:0,fontWeight:900 }}>O</span>RA</span>
        </div>
  ...
  return (
    <style>{`
      ...
      *,*::before,*::after { box-sizing:border-box; margin:0; padding:0; }
      ...
      .section-fade { animation: fadeUp 0.45s cubic-bezier(.22,.68,0,1.2); }
      .card-enter   { animation: fadeUp 0.35s ease both; }
      /* Smooth card hover */
      img { transition: transform .4s ease, filter .3s ease; }
      button { transition: background .18s, color .18s, border-color .18s, box-shadow .18s, transform .15s; }
      ...
      /* Hero crossfade */
      .hero-img { transition: opacity .8s ease; }
